      ...
      <div class="span12">
        ...
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Expense table</h5>
           <a href="{{ route ('expense.create') }}" span class="label label-info"><i class="icon-plus icon-white"></i> Add New Expenses</span> </a>
          </div>
            ...
            <table class="table table-bordered data-table">
              ...
              <tbody>
                ...
                <tr class="gradeX">
                    ...
                    <td>{{ $loop->index + 1 }}</td>
                      <td>
                      @if (!empty ( $expenses->expense_type))
                      {{ $expenses->expense_type->name }}
                      @endif
                      </td>
                      <td>{{ $expenses->amount }}</td>
                      <td>{{ $expenses->files}} </td>
                      <td>{{ $expenses->notes}}</td>
                      <td>{{ $expenses->date}} </td>
                      

                      <td>
                       <li><a href="{{ route('expense.edit', $expenses->id) }}"><i class="icon-pencil"></i>Edit</a></li>

                          <form id="del-form-{{ $expenses->id }}" method="post" action="{{ route('expense.destroy', $expenses->id) }}">
                            {{csrf_field() }}
                            {{ method_field('DELETE') }}
                              <li><a href="" onclick="
                                event.preventDefault();
                                document.getElementById('del-form-{{ $expenses->id }}').submit();">
