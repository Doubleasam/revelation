      ...
      <div class="span12">
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <form class="form-horizontal" method="POST" action="{{ route ('followup.store') }}" id="basic_validate" novalidate="novalidate" enctype="multipart/form-data">
              {{ csrf_field() }}
                  ...
                  <div class="control-group">
                    <label class="control-label">Follow Up Category</label>
                    <div class="controls">
                      <select name="follow_up_category_id">
                           @foreach ($followupcat as $followupcats)
                            <option value="{{ $followupcats->id }}">{{ $followupcats->name }}</option>
                            @endforeach
                        </select>
                    </div>
                  </div>
                  ...
                  <!-- <div class="control-group">
                    ...
                    <div class="controls">
                      <select name="hod_id">
                           @foreach ($hod as $hods)
                            <option value="{{ $hods->first_name }}">{{ $hods->first_name  }}</option>
                            @endforeach
                        </select>
                    </div>
                  </div> -->
